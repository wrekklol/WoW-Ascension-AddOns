...
function set.getpoints(name, map)
    --print("set.getpoints name="..tostring(name).." map="..tostring(map))
    if(GatherMate) then
        --print("GatherMate found, continuing")
        --local gmdbs = GatherMate:GetNodesForZone(Yatlas_WorldMapIds[map],"Mining",true)
        --print("Silithus="..GatherMate:GetZoneID("Silithus").." "..tostring(Yatlas_WorldMapIds[map]))
        for a,b in pairs(Yatlas_PaperZoneNames[map]) do
            --print("a="..tostring(a).." b="..tostring(b))
            for zid,zdat in GatherMate:GetNodesForZone(b,"Mining",true) do
            --for zid,zdat in GatherMate:GetNodesForZone("Silithus","Mining",true) do
                --print("zid="..tostring(zid).." zdat="..tostring(zdat))
                local zname = Yatlas_PaperZoneNames[map][zid];
                --print("zname="..tostring(zname))
                if(zname ~= nil) then
                    local real_zid;
